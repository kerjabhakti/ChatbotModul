...
const start = async (client = new Client()) => {
        ...
        client.onAddedToGroup(((chat) => {
            ...
            if (totalMem < 257) { 
            	client.sendText(chat.id, `Halo warga ${name} mohon maaf, jika mau menginvite bot, silahkan membeli akses premium bot melalui wa.me//6281374347075`).then(() => client.leaveGroup(chat.id)).then(() => client.deleteChat(chat.id))
            } else {
    ...
    await client.sendText(chat.id, `Sorry, bot tidak bisa join di grub untuk menginvite bot silahkan hubungi owner bot https://wa.me/ max group: ${groupLimit}`).then(() => {
    ...
    // kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await client.simulateTyping(chat.id, true).then(async () => {
          await client.sendText(chat.id, `Hai minna~, Im ITEUNG V2 BOT. To find out the commands on this bot type #menu`)
        })
        }
    }
    })
